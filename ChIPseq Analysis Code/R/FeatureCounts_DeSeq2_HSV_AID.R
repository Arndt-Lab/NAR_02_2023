...
fCount1 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Arndt_Lab_Consulting/ChIPseq_Redo_Spt6_AID_112022/DESeq_Analysis_ChIPseq_Redo_11192022/bams/SN090_S9_nodups.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
fCount2 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Arndt_Lab_Consulting/ChIPseq_Redo_Spt6_AID_112022/DESeq_Analysis_ChIPseq_Redo_11192022/bams/SN091_S10_nodups.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
fCount3 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Arndt_Lab_Consulting/ChIPseq_Redo_Spt6_AID_112022/DESeq_Analysis_ChIPseq_Redo_11192022/bams/SN092_S11_nodups.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
fCount4 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Arndt_Lab_Consulting/ChIPseq_Redo_Spt6_AID_112022/DESeq_Analysis_ChIPseq_Redo_11192022/bams/SN093_S12_nodups.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
...
conditions <- c("HSV_DMSO", "HSV_DMSO",
                "HSV_IAA", "HSV_IAA")
...
rownames(coldata) <- (c("HSV_DMSO", "HSV_DMSO",
                        "HSV_IAA", "HSV_IAA"))
...
#dds <- DESeq(dds)
resultsNames(dds)
sizeFactors(dds) <- c(1/0.322083, 1/0.204436, 1/0.139225, 1/0.114162)

dds = estimateDispersions(dds) 
dds = nbinomWaldTest(dds)

#!!!input contrasting conditions here 
contrast=c("condition","HSV_IAA","HSV_DMSO")
HSV_IAA_vs_HSV_DMSO <- results(dds, contrast, pAdjustMethod = "fdr")

write.table(subset(HSV_IAA_vs_HSV_DMSO, padj < 0.05), file="HSV_IAA_vs_HSV_DMSO_significant.tsv", 
            sep = "\t", quote = FALSE, row.names = TRUE, col.names = TRUE)


write.table(subset(HSV_IAA_vs_HSV_DMSO, log2FoldChange < -0.58), file="HSV_IAA_vs_HSV_DMSO_l2fc_less_than_-0.58.tsv", 
            sep = "\t", quote = FALSE, row.names = TRUE, col.names = TRUE)



#output is a results file comparing two conditions
png("HSV_IAA_vs_HSV_DMSO_MA_plot.png", height = 600, width = 600)
plotMA(HSV_IAA_vs_HSV_DMSO, ylim=c(-5,5), alpha = 0.05, xlab = "NCIS Normalized Read Counts")
dev.off()
