...
fCount1 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Arndt_Lab_Consulting/ChIPseq_Redo_Spt6_AID_112022/DESeq_Analysis_ChIPseq_Redo_11192022/bams/SN094_S13_nodups.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
fCount2 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Arndt_Lab_Consulting/ChIPseq_Redo_Spt6_AID_112022/DESeq_Analysis_ChIPseq_Redo_11192022/bams/SN097_S16_nodups.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
fCount3 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Arndt_Lab_Consulting/ChIPseq_Redo_Spt6_AID_112022/DESeq_Analysis_ChIPseq_Redo_11192022/bams/SN096_S15_nodups.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
fCount4 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Arndt_Lab_Consulting/ChIPseq_Redo_Spt6_AID_112022/DESeq_Analysis_ChIPseq_Redo_11192022/bams/SN095_S14_nodups.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
...
conditions <- c("8WG16_DMSO", "8WG16_DMSO",
                "8WG16_IAA", "8WG16_IAA")
...
rownames(coldata) <- (c("8WG16_DMSO", "8WG16_DMSO",
                        "8WG16_IAA", "8WG16_IAA"))
...
#dds <- DESeq(dds)
resultsNames(dds)
sizeFactors(dds) <- c(1/0.116088, 1/0.1128, 1/0.175085, 1/0.12762)

dds = estimateDispersions(dds) 
dds = nbinomWaldTest(dds)

#!!!input contrasting conditions here 
contrast=c("condition","8WG16_IAA","8WG16_DMSO")
a8WG16_IAA_vs_8WG16_DMSO <- results(dds, contrast, pAdjustMethod = "fdr")

write.table(subset(a8WG16_IAA_vs_8WG16_DMSO, padj < 0.05), file="8WG16_IAA_vs_8WG16_DMSO_significant.tsv", 
            sep = "\t", quote = FALSE, row.names = TRUE, col.names = TRUE)


write.table(subset(a8WG16_IAA_vs_8WG16_DMSO, log2FoldChange < -0.58), file="8WG16_IAA_vs_8WG16_DMSO_l2fc_less_than_-0.58.tsv", 
            sep = "\t", quote = FALSE, row.names = TRUE, col.names = TRUE)



#output is a results file comparing two conditions
png("8WG16_IAA_vs_8WG16_DMSO_MA_plot.png", height = 600, width = 600)
plotMA(a8WG16_IAA_vs_8WG16_DMSO, ylim=c(-5,5), alpha = 0.05, xlab = "NCIS Normalized Read Counts")
dev.off()
