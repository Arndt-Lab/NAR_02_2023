...
fCount1 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Arndt_Lab_Consulting/ChIPseq_Redo_Spt6_AID_112022/DESeq_Analysis_ChIPseq_Redo_11192022/bams/SN086_S5_nodups.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
fCount2 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Arndt_Lab_Consulting/ChIPseq_Redo_Spt6_AID_112022/DESeq_Analysis_ChIPseq_Redo_11192022/bams/SN087_S6_nodups.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
fCount3 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Arndt_Lab_Consulting/ChIPseq_Redo_Spt6_AID_112022/DESeq_Analysis_ChIPseq_Redo_11192022/bams/SN088_S7_nodups.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
fCount4 <- featureCounts(files="/Volumes/Mitch_Home_Backup/Arndt_Lab_Consulting/ChIPseq_Redo_Spt6_AID_112022/DESeq_Analysis_ChIPseq_Redo_11192022/bams/SN089_S8_nodups.bam",
                         annot.ext=annotFile, isGTFAnnotationFile=TRUE, GTF.featureType=feature, GTF.attrType="gene_id")
...
rownames(coldata) <- (c("V5_DMSO", "V5_DMSO",
                        "V5_IAA", "V5_IAA"))
...
#dds <- DESeq(dds)
resultsNames(dds)
sizeFactors(dds) <- c(1/0.0378407, 1/0.0317041, 1/0.0142405, 1/0.0132246)

dds = estimateDispersions(dds) 
dds = nbinomWaldTest(dds)

#!!!input contrasting conditions here 
contrast=c("condition","V5_IAA","V5_DMSO")
V5_IAA_vs_V5_DMSO <- results(dds, contrast, pAdjustMethod = "fdr")

write.table(subset(V5_IAA_vs_V5_DMSO, padj < 0.05), file="V5_IAA_vs_V5_DMSO_significant.tsv", 
            sep = "\t", quote = FALSE, row.names = TRUE, col.names = TRUE)


write.table(subset(V5_IAA_vs_V5_DMSO, log2FoldChange < -0.58), file="V5_IAA_vs_V5_DMSO_l2fc_less_than_-0.58.tsv", 
            sep = "\t", quote = FALSE, row.names = TRUE, col.names = TRUE)



#output is a results file comparing two conditions
png("V5_IAA_vs_V5_DMSO_MA_plot.png", height = 600, width = 600)
plotMA(V5_IAA_vs_V5_DMSO, ylim=c(-5,5), alpha = 0.05, xlab = "NCIS Normalized Read Counts")
dev.off()
